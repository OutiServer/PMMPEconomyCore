...
class SetMoney implements BaseForm
{
    public const FORM_KEY = "economy_form_setmoney";

    public function execute(Player $player): void
    {
        $form = new CustomForm(EconomyCore::getInstance(),
            $player,
            LanguageManager::getInstance()->getLanguage($player->getLocale())->translateString("form.setmoney.title"),
            [
                new ContentInput(LanguageManager::getInstance()->getLanguage($player->getLocale())->translateString("form.setmoney.button1"), "playerName"),
                new ContentInput(LanguageManager::getInstance()->getLanguage($player->getLocale())->translateString("form.setmoney.button2"), "addMoney", inputType: ContentInput::TYPE_INT)
            ],
